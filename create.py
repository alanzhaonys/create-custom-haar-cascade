# Original GITHUB: https://github.com/EvilPort2/Create-Custom-Haar-Cascade
# Dataset from: https://github.com/chandrikadeb7/Face-Mask-Detection
...
def create_samples(inp_dir, num):
    for file in os.listdir(inp_dir):
        cmd = "opencv_createsamples -img " + inp_dir + "/" + file + " -bg bg.txt -info positives/info" + \
              file.split('.')[0] + ".lst -pngoutput positives -maxxangle 0.5 -maxyangle 0.5 -maxzangle 0.5 -num " + num
        os.system(cmd)
...
while True:
    ...
    print(
        "4. Create positive images (samples) from negative and input images\n5. Start training Haar Cascades\n6. Cleanup all")
    ...
    if choice == 1:
        ...
        print("-------------------------------------------\n")
        url = str(input(
            "Enter the URL that contains the download links (should look like http://image-net.org/api/text/imagenet.synset.geturls?wnid=n03642806): "))
        ...
        input("Put the one of each ugly images in the " + ugly_folder + " folder and press ENTER....")
        ...
        resize_x = int(input("Enter the resize dimension in x-axis (recommended 50): "))
        resize_y = int(input("Enter the resize dimension in y-axis (recommended 50): "))
        ...
        pos = input("Enter number of positive samples you want to use for training: ")
        neg = input("Enter number of negative samples you want to use for training: ")
        num = input("Enter number of stages: ")
        ...
        print("-----------\n")
        print(
            "WARNING: This will cleanup the following:-\ninput_images folder\ninput_resized_images folder\nbg.txt\npositives folder\npositive.vec file\nuglies folder\n")
